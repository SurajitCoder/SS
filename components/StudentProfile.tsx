...
const StudentProfile: React.FC<StudentProfileProps> = ({ 
  ...
  const paidCount = student.paidMonths?.filter(m => m.startsWith(CURRENT_YEAR.toString())).length || 0;
        ...
        <div className="bg-brightx-navy p-8 md:p-12 relative shrink-0">
          ...
          <div className="flex flex-wrap items-center justify-center md:justify-end gap-3 mt-10">
            <button 
              ...
              className="flex items-center gap-2 px-6 py-3 bg-brightx-teal text-white rounded-2xl font-black text-[10px] uppercase tracking-widest hover:brightness-110 transition-all shadow-lg"
            >
              <FileText className="w-4 h-4" /> Report Card
            </button>
            <button onClick={onPromote} className="flex items-center gap-2 px-6 py-3 bg-emerald-500 text-white rounded-2xl font-black text-[10px] uppercase tracking-widest hover:brightness-110 transition-all">
            ...
            <button onClick={onDeactivate} className="flex items-center gap-2 px-6 py-3 bg-orange-500 text-white rounded-2xl font-black text-[10px] uppercase tracking-widest hover:brightness-110 transition-all">
              <UserX className="w-4 h-4" /> Archive
            </button>
            <button onClick={onDelete} className="p-3 bg-red-500 text-white rounded-2xl hover:brightness-110 transition-all">
          ...
          <div className="md:col-span-4 space-y-6">
            <div className="bg-white p-8 rounded-[2rem] shadow-sm border border-gray-100 h-fit">
              <div className="flex items-center gap-3 mb-6">
                ...
                <h3 className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Registry Information</h3>
              </div>
              <div className="space-y-4 mb-8">
                <div>
                  <p className="text-[9px] font-black text-gray-300 uppercase tracking-widest mb-1">Guardian Name</p>
                  <p className="font-black text-gray-800 text-sm">{student.guardianName || 'N/A'}</p>
                </div>
                <div>
                  <p className="text-[9px] font-black text-gray-300 uppercase tracking-widest mb-1">Guardian Phone</p>
                  <p className="font-black text-gray-800 text-sm">{student.guardianPhone || 'N/A'}</p>
                </div>
                <div>
                  <p className="text-[9px] font-black text-gray-300 uppercase tracking-widest mb-1">Student Contact</p>
                  <p className="font-black text-gray-800 text-sm">{student.studentPhone}</p>
                </div>
                <div>
                  <p className="text-[9px] font-black text-gray-300 uppercase tracking-widest mb-1">Enrollment Date</p>
                  <p className="font-black text-gray-800 text-sm">{student.admissionDate || 'N/A'}</p>
                </div>
                <div>
                  <p className="text-[9px] font-black text-gray-300 uppercase tracking-widest mb-1">Address Detail</p>
                  <p className="font-bold text-gray-700 text-xs leading-relaxed">{student.address || 'Not Provided'}</p>
                </div>
              </div>
          ...
          <div className="md:col-span-4 space-y-6">
            <div className="bg-brightx-navy p-8 rounded-[2rem] shadow-xl text-white min-h-[180px] flex flex-col">
              <div className="flex items-center gap-3 mb-6 opacity-40">
                ...
                <h3 className="text-[10px] font-black uppercase tracking-widest">Exam Snapshot</h3>
              </div>
            ...
            <div className="bg-white p-8 rounded-[2rem] shadow-sm border border-gray-100">
              ...
              <div className="flex justify-between items-center mb-8">
                <div className="text-center px-4">
                  <p className="text-4xl font-black text-gray-800">{student.attendance?.length || 0}</p>
                  <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mt-1">Days Present</p>
                </div>
                <div className="text-center px-4">
                  <p className="text-4xl font-black text-red-500">{student.absences?.length || 0}</p>
